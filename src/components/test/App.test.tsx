...
import { GET_MEAL_TITLES } from '../molecules/Queries';
...
const mealTitlesMock = [
    {
      request: {
        query: GET_MEAL_TITLES,
        variables: {}, 
      },
      result: {
        data: { mealTitles: ['Kafteji', 'Leblebi Soup'] },
      },
    },
  ];
...
test('renders the app without errors', async () => {
    const { getByTestId, findByTestId } = render(
        <MockedProvider mocks={[...mocks, ...mealTitlesMock]} addTypename={false}>
            <App />
        </MockedProvider>
    );

    // Use findByTestId instead of findAllByTestId
    const headerElement = getByTestId('test-header'); 
    const filterSectionElement = getByTestId('test-filter'); 
    const footerElement = getByTestId('cypress-footer');

    // Use findByTestId to wait for the elements to appear
    await findByTestId('test-header');
    await findByTestId('test-filter');
    await findByTestId('cypress-footer');
