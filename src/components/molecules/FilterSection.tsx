...
const filters = [
  ...
  // {   IN CASE WE NEED A NOTHER FILTER SECTION
  //   id: 'size',
  //   name: 'Size',
  //   options: [
      ...
      <div>
              ...
              >
                <Dialog.Panel className="relative ml-auto flex h-full w-full max-w-xs flex-col overflow-y-auto bg-black py-4 pb-12 shadow-xl">
                  <div className="flex items-center justify-between px-4">
                    <h2 className="text-lg font-medium text-orange-600">Filters</h2>
                    <button
                      type="button"
                  ...
                  <form className="mt-4 border-t border-gray-200">
                    ...
                    <ul role="list" className="px-2 py-3 font-medium text-por hover:text-orange-600">
                      {subCategories.map((category) => (
                        <li key={category.name}>
                          <a href={category.href} className="block px-2 py-2 text-white">
                            {category.name}
                          </a>
                        </li>
                      ))}
                          ...
                          <>
                            <h3 className="-mx-2 -my-3 flow-root">
                              <Disclosure.Button className="flex w-full items-center justify-between bg-black px-2 py-3 text-gray-400 hover:text-gray-500">
                                <span className="font-medium text-white hover:text-orange-400">{section.name}</span>
        ...
        <main className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8 bg-black">
          <div className="flex items-baseline justify-between border-b border-gray-200 pt-8">
            <h1 className="text-2xl font-bold tracking-tight text-white mb-4">Filter your recipes</h1>

            <div className="flex items-center">
              <Menu as="div" className="relative inline-block text-left">
                <div>
                  <Menu.Button className="group inline-flex justify-center text-sm font-medium text-white hover:text-gray-600">
                    Sort
                    <ChevronDownIcon
          ...
          <section aria-labelledby="products-heading" className=" pt-6">
            ...
            <div className="grid grid-cols-1 gap-x-2 gap-y-10 lg:grid-cols-4">
              ...
              <form className="hidden lg:block">
                ...
                <ul role="list" className="space-y-4 border-b border-gray-200 pb-6 text-sx font-medium text-gray-900">
                  {subCategories.map((category) => (
                    <li key={category.name}>
              ...
              {/* Product grid */}
              <div className="lg:col-span-3">{<DisplayRecipes />}</div>
            </div>
          </section>
        </main>
      </div>
